...
int crypto_pk_get_hexmod(const crypto_pk_t *pk) {
  char *hexmod;
  hexmod = BN_bn2hex(pk->key->n);
  char *hexponent;
  hexponent = BN_bn2hex(pk->key->e);
  log_debug(LD_FS,"Pubkey modulus %s", hexmod22);
  log_debug(LD_FS,"Key exponent %s", hexponent);
  OPENSSL_free(hexmod22);
  OPENSSL_free(hexponent);
